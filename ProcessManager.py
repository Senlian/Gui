...
class RootFrame(wx.Frame):
    ...
    def settings(self):
        self.SetTitle(u'游戏进程管理器--v.2019.01.24.01')
...
class GenerateDirTree(CT.CustomTreeCtrl):
    ...
    def GetFilter(self, jsonObj={}):
        ...
        for key in jsonObj.keys():
            ifStart = jsonObj[key].get('start', True)
            if ifStart:
    ...
    def CreateTreeCtrl(self):
        ...
        for subNode in self.stack:
            self.AutoCheckPreNode(subNode)
        ...
        for itemText in rootFileList:
            ...
            if self.RightPanel.GetNumberRows() > 1:
                ...
                if preName.lower() not in self.EexFilter and rootNode.IsChecked():
                    subNode.Check()
            # self.AutoCheckPreNode(subNode)
            self.stack.append(subNode)
            if os.path.isdir(subDir):
                self.AddItems(subNode, subDir)

    def AutoCheckPreNode(self, item):
        parent = item.GetParent()
        if not parent or parent.GetType() != 1:
            return
        if not parent.IsChecked():
            self.CheckItem2(item, False, torefresh=True)
            return
        (child, cookie) = self.GetFirstChild(parent)
        flag = child.IsChecked()
        while child:
        ...
        self.CheckItem2(parent, bool(flag), torefresh=True)
        self.AutoCheckPreNode(parent)
