...
def SaveFile(targetFile, newText, saveAs=False):
    if not os.path.isfile(targetFile) and not saveAs:
        ...
        #     return True
        with openFile(targetFile, 'w', encoding='GB2312') as f:
...
class MyFrame(wx.Frame):
    ...
    def initUI(self):
        ...
        self.TopToolBar()
        self.BottomStatuBar()
        thread_main = threading.Thread(target=self.MidWindow)
...
class MyThread(Thread):
    ...
    def run(self):
        ...
        for curFile in self.FileList:
            ...
            csvFP.close()
            LocalTemplate.save(curFile, 'GB2312')
